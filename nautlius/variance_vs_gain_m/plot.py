...
def run_once(variance, M):
    ...
    return meanSE(alpha_opt, r_opt, cx_opt, cy_opt, a_opt, M=M, users=users)

def variance_gain(iterations=100):
    ...
    for it in range(iterations):
        ...
        m1_results = Parallel(n_jobs=-1)(
            delayed(run_once)(variance, 1) for variance in variance_arr
        )
        m1.append(m1_results)

        m2_results = Parallel(n_jobs=-1)(
            delayed(run_once)(variance, 2) for variance in variance_arr
        )
        m2.append(m1_results)

        m3_results = Parallel(n_jobs=-1)(
            delayed(run_once)(variance, 3) for variance in variance_arr
        )
        m3.append(m1_results)

        m4_results = Parallel(n_jobs=-1)(
            delayed(run_once)(variance, 4) for variance in variance_arr
        )
        m4.append(m1_results)

        m5_results = Parallel(n_jobs=-1)(
            delayed(run_once)(variance, 5) for variance in variance_arr
        )
        m5.append(m1_results)
    ...
    m3_mean = m3.mean(axis=0)
    m4_mean = m4.mean(axis=0)
    m5_mean = m5.mean(axis=0)

    plt.plot(variance_arr, m1_mean, c='b', label='M = 1')
    plt.plot(variance_arr, m2_mean, c='g', label='M = 2')
    plt.plot(variance_arr, m3_mean, c='r', label='M = 3')
    ...
    np.savez('variance-vs-gain-vs-m-data.npz',
